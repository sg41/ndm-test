...
func TestQueue(t *testing.T) {
	...
	// PUT → 200
	w = httptest.NewRecorder()
	h(w, req("PUT", "/test", "v=a"))
	if w.Code != 200 {
		t.Errorf("PUT a: got %d", w.Code)
	}
	w = httptest.NewRecorder()
	h(w, req("PUT", "/test", "v=b"))
	if w.Code != 200 {
	...
	go func() {
		...
		h(w2, req("PUT", "/t2", "v=late"))
	}()
	w = httptest.NewRecorder()
	h(w, req("GET", "/t2", "timeout=2"))
	...
	// Два PUT подряд
	h(httptest.NewRecorder(), req("PUT", "/t4", "v=first"))
	h(httptest.NewRecorder(), req("PUT", "/t4", "v=second"))
	wg.Wait()
	if !done1 || !done2 {
...
func TestConcurrentPutGet(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func(n int) {
			defer wg.Done()
			w := httptest.NewRecorder()
			h(w, req("PUT", "/conc", "v="+string(rune('0'+n))))
		}(i)
		go func() {
			defer wg.Done()
			w := httptest.NewRecorder()
			h(w, req("GET", "/conc", "timeout=1"))
		}()
	}
	wg.Wait()
